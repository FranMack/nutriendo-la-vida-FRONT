...
const menu=[{title:"Home",path:"#home"},{title:"Conocerme",path:"#me"},{title:"Planes",path:"#plans"},{title:"Ebook",path:"#ebook"}]
...
export const Navbar = () => {
 ...
 <div className='logo-container' onClick={linkToHome}><img src={logoN} alt="letter N" /><img src={logoV} alt="letter V" /></div>
        
        <ul className='navbar-menu'>
            {menu.map((item)=>{return(
                <a href={item.path==="#home"?"/":`/${item.path}`}><li  key={item.title}>{item.title}</li></a>
            )})}
        ...
        {location!=="test" &&<button className='navbar-button' onClick={linkToTest}>
            Hacer test
        </button>}
    </nav>
  )
